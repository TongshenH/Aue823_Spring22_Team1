...
class Final(object):
    def __init__(self):
        ...
        self.yolo_model = torch.hub.load('/home/tongshen/.local/lib/python3.8/site-packages/yolov5', 'custom',
                                         path='/home/tongshen/Projects/AuE8230Spring23_Tongshen/catkin_ws/src/auefinals/src/yolov5s.pt',
                                         source='local')
        print('finished init')
        ...
        self.apriltag_subscriber = rospy.Subscriber('/tag_detections', AprilTagDetectionArray, self.apriltag_callback)
        ...
        self.hsvMin = (20,39,113)
        self.hsvMax = (79,185,231)
    ...
    def lidar_callback(self, msg):
        self.left = self.normalize_lidar(list(msg.ranges[-60:-3]))
        self.right = self.normalize_lidar(list(msg.ranges[3:60]))
    ...
    def avoid_obstacles(self):
        ...
        print('Left:', sum(self.left))
        print('right:', sum(self.right))
    ...
    def convert_binary(self):
        ...
        # apply thresholds
        mask = cv2.inRange(hsv_image, self.hsvMin, self.hsvMax)
    ...
    def apriltag_callback(self, msg):
        if not msg:
            ...
            self.x = msg.detections[0].pose.pose.pose.position.x
            self.z = msg.detections[0].pose.pose.pose.position.z

    def follow_apriltag(self):
        # Initial angular velocity and linear velocity
        k_linear_vel = 0.1
        k_angular_vel = 0.5
        self.vel_msg.linear.x = self.z * k_linear_vel
        self.vel_msg.angular.z = -self.x * k_angular_vel
    ...
    def run(self):
        ...
        with keyboard.Events() as events:
            ...
            while not rospy.is_shutdown():
                ...
                elif self.pressed_button == 'a':
                    print('a pressed')
                    self.mode = 'apriltag'
                elif self.pressed_button == 'c':
                    self.hsvMin = (16, 60, 68)
                    self.hsvMax = (90, 196, 177)
