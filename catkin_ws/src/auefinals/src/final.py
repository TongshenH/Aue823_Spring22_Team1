...
class Final(object):
    def __init__(self):
        ...
        self.yolo_model = torch.hub.load('/home/tongshen/.local/lib/python3.8/site-packages/yolov5', 'custom',
                                         path='/home/tongshen/Projects/AuE8230Spring23_Tongshen/catkin_ws/src/auefinals/yolov5s.pt',
                                         source='local')
    ...
    def camera_callback(self, msg):
        try:
            self.cv2_image = cv2.resize(self.bridge_object.compressed_imgmsg_to_cv2(msg, desired_encoding="bgr8"),
                                        (400, 400), interpolation=cv2.INTER_AREA)
        ...
        while not rospy.is_shutdown():
            ...
            if xmin != -1:
                ...
                if bounding_box_size > threshold and self.stopped == False:
                    ...
                    self.vel_msg.linear.x = self.starting_vel
                    self.vel_publisher.publish(self.vel_msg)
            # if find the Apriltag, execute apriltag following
            if self.z:
